    ...
    with st.container(border=True, height=540):
        st.markdown("""
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; border-bottom: 1px solid #F1F5F9; padding-bottom: 10px;">
                <span style="font-weight:800; font-size:16px; color:#1F2937; text-transform:uppercase; letter-spacing:0.04em;">🛠️ Dispatch Command Center</span>
                <span style="font-size:12px; background:#F0FDF4; color:#16A34A; padding:4px 8px; border-radius:8px; font-weight:700; border: 1px solid #DCFCE7;">ACTIVE COMMAND</span>
            </div>
        """, unsafe_allow_html=True)
       
        # Hotspot selection dropdown
        h_names = [h['street'] for h in active_city_data['hotspots']]
        # Find index of currently active hotspot
        curr_id = st.session_state.active_report_id
        curr_idx = next((i for i, h in enumerate(active_city_data['hotspots']) if h['id'] == curr_id), 0)
       
        selected_street = st.selectbox(
            "SELECT TARGET ANOMALY LOGS SITE",
            h_names,
            index=curr_idx,
            label_visibility="collapsed"
        )
       
        # Update active hotspot in session state
        h_selected = next(h for h in active_city_data['hotspots'] if h['street'] == selected_street)
        if h_selected['id'] != st.session_state.active_report_id:
            st.session_state.active_report_id = h_selected['id']
            st.session_state.view_action_details = h_selected['id']
            st.rerun()
           
        c_status = st.session_state.action_statuses.get(h_selected['id'], "🟡 Pending Approval")
       
        # AI Source Verification Fused Cards
        st.markdown(f"""
            <div style="font-size: 13px; font-weight:700; color:#64748B; text-transform:uppercase; margin-bottom:8px;">🛰️ AI Sensor Fusion verification</div>
            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap:6px; margin-bottom:14px;">
                <div style="background:#F1F5F9; border-radius:6px; padding:6px; text-align:center; border: 1px solid #E2E8F0;">
                    <div style="font-size:10px; color:#64748B;">CITIZEN</div>
                    <div style="font-weight:800; color:#1E293B;">{h_selected['cit_conf']}</div>
                </div>
                <div style="background:#F1F5F9; border-radius:6px; padding:6px; text-align:center; border: 1px solid #E2E8F0;">
                    <div style="font-size:10px; color:#64748B;">SATELLITE</div>
                    <div style="font-weight:800; color:#1E293B;">{h_selected['sat_conf']}</div>
                </div>
                <div style="background:#F1F5F9; border-radius:6px; padding:6px; text-align:center; border: 1px solid #E2E8F0;">
                    <div style="font-size:10px; color:#64748B;">SENSOR</div>
                    <div style="font-weight:800; color:#1E293B;">{h_selected['sens_conf']}</div>
                </div>
            </div>
           
            <div style="background:#F8FAFC; border:1px solid #E2E8F0; padding:12px; border-radius:10px; margin-bottom:14px;">
                <div style="display:flex; justify-content:space-between; margin-bottom:6px;">
                    <span style="font-size:13px; font-weight:700; color:#475569;">Mitigation Action Directive</span>
                    <span style="color:#2563EB; font-weight:800; font-size:12px; background:#EFF6FF; padding:2px 6px; border-radius:4px;">PROPOSED</span>
                </div>
                <div style="font-size:16px; font-weight:800; color:#0F172A; margin: 4px 0;">{h_selected['act']}</div>
                <div style="font-size:13px; color:#475569; display:grid; grid-template-columns: 1fr 1fr; gap:6px; padding-top:6px; border-top:1px dashed #E2E8F0;">
                    <span>⏱️ ETA: <b>{h_selected['eta']}</b></span>
                    <span>📉 Reduction: <b>{h_selected['red']}</b></span>
                    <span style="grid-column:span 2;">🚒 Dispatch Crew: <b>{h_selected['team']}</b></span>
                </div>
            </div>
        """, unsafe_allow_html=True)
        ...
        with btn_col1:
            if st.button("🚀 Dispatch Team", key=f"p1_dispatch_{h_selected['id']}", use_container_width=True):
                st.session_state.action_statuses[h_selected['id']] = "🟢 Approved"
                st.toast(f"Mobilized Mitigation Plan: dispatched {h_selected['team']} to {h_selected['street']}.")
                st.rerun()
        with btn_col2:
            if st.button("✓ Mark Mitigated", key=f"p1_resolved_{h_selected['id']}", use_container_width=True):
                st.session_state.action_statuses[h_selected['id']] = "🔵 Completed"
                st.toast(f"Marked action anomaly resolved at {h_selected['street']}.")
                st.rerun()
               
        # Show Current Status and confidence level indicators
        st.markdown(f"""
            <div style="margin-top:12px; background:#FFFBEB; border:1px solid #FDE68A; padding:10px; border-radius:8px; display:flex; justify-content:space-between; align-items:center;">
                <span style="font-size:13px; font-weight:700; color:#B45309;">CURRENT STATUS</span>
                <span style="font-size:13px; font-weight:800; color:#78350F;">{c_status}</span>
            </div>
        """, unsafe_allow_html=True)
...
with st.container(border=True):
    ...
    with col1:
        st.markdown("""
            <div style="background: #FEF2F2; border: 1px solid #FEE2E2; border-left: 5px solid #EF4444; padding: 20px; border-radius: 12px;">
...
with st.container(border=True):
    ...
    action_cols = st.columns(len(active_city_data['hotspots']))
    for idx, hs in enumerate(active_city_data['hotspots']):
...
with st.container(border=True):
    ...
    with intel_col1:
        st.markdown(f"""
            <div style="background: #F0FDF4; border: 1px solid #DCFCE7; border-left: 5px solid #16A34A; padding: 20px; border-radius: 12px; font-size: 16px; min-height: 180px;">
    ...
    with intel_col2:
        st.markdown(f"""
            <div style="background: #F8FAFC; border: 1px solid #E2E8F0; border-left: 5px solid #64748B; padding: 20px; border-radius: 12px; font-size: 16px; min-height: 180px;">
    ...
    with intel_col3:
        st.markdown(f"""
            <div style="background: #EFF6FF; border: 1px solid #BFDBFE; border-left: 5px solid #2563EB; padding: 20px; border-radius: 12px; font-size: 16px; min-height: 180px;">
